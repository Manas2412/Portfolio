    ...
    <div className="relative w-full overflow-hidden" style={maskStyle}>
      ...
      >
        {duplicated.map((skill, idx) => (
          <div
            key={`${skill.name}-${idx}`}
            className="flex items-center gap-3 px-5 py-3 bg-white/[0.03] border border-white/[0.08] rounded-xl hover:bg-white/[0.07] hover:border-white/20 transition-all duration-300 min-w-max"
          >
            <skill.icon className="w-4 h-4 text-purple" />
            <span className="text-white-100 text-sm font-medium tracking-wide">{skill.name}</span>
          </div>
        ))}
      </motion.div>
    </div>
        ...
        >
          ...
          <div className="space-y-10 mt-16">
            {sections.map(({ label, items, direction, speed }) => (
              ...
              >
                <p className="text-xs uppercase tracking-widest text-white-200 opacity-50 px-1">
                  {label}
                </p>
